...
export default function Button({ children, dataBSPlacement, onClick, shape, text, title }) {
  return (
    <button
      ...
      className={` ${
        shape === 'rounded'
          ? 'absolute bg-violet-300 hover:bg-violet-500 p-2 bottom-2 right-2 lg:bottom-4 lg:right-4 cursor-pointer rounded-full opacity-0 group-hover:opacity-100 checked:opacity-100'
